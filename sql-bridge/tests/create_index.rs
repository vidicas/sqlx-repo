...
fn create_index_auto_named() {
    ...
    assert_eq!(error, Error::CreateIndex { reason: "nameless" });
...
fn create_index_using_index_method() {
    ...
    let error = parse(input).unwrap_err();
    assert_eq!(error, Error::CreateIndex { reason: "using" });
...
fn create_index_with_include() {
    ...
    let error = parse(input).unwrap_err();
    assert_eq!(error, Error::CreateIndex { reason: "include" });
...
fn create_index_with() {
    ...
    let error = parse(input).unwrap_err();
    assert_eq!(error, Error::CreateIndex { reason: "with" });
